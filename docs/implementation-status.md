...
- 다음 배포 방향은 Vercel + Supabase다. Supabase snapshot adapter·RLS migration·Vercel catch-all handler wiring을 구현했고, 실제 Supabase project/Vercel preview와 multi-instance stream은 아직 검증하지 않았다.
- 현재 Vercel CLI는 설치되어 있으나 로그아웃 상태이며, Supabase CLI는 현재 `linux-x64` 바이너리 패키지를 제공하지 않아 외부 project provisioning/migration/deploy는 blocker다. 인증·project/region/plan 승인 전에는 배포하지 않는다.
...
- 검증: `npm run check`, `npm run security`, `npm test` 131/131 통과. `npm test`는 HTTP child-process startup 경합을 피하기 위해 Node test concurrency 1로 실행한다.
