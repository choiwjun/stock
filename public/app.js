...
function sourceLabel(source) {
  ...
  if (/demo-fixture|샌드박스/i.test(normalized)) return "예시 데이터";
  return normalized;
}

function freshness(status, asOf, source = "") {
  const normalized = String(status || "UNAVAILABLE").toLowerCase();
  return `<span class="freshness ${normalized}"><span aria-hidden="true">●</span>${escapeHtml(STATUS_LABELS[status] || status || "상태 확인 필요")}<span class="sr-only">, 기준 시각 ${escapeHtml(formatDateTime(asOf))}</span></span>${source ? `<span class="muted small">${escapeHtml(sourceLabel(source))}</span>` : ""}`;
}
...
function routeKey(route) {
  return `${route.name}:${route.ticker || ""}`;
}

function navIsActive(route, name) {
  return route.name === name || (name === "market" && route.name === "stock");
}

function accountActionsMarkup() {
  return state.session.authenticated
    ? `<span class="account-chip"><span class="status-dot" aria-hidden="true"></span>${escapeHtml(ROLE_LABELS[state.role] || "회원")}</span><button class="btn btn-quiet" type="button" data-action="logout">로그아웃</button>`
    : `<button class="btn btn-secondary btn-login" type="button" data-action="sandbox-login">로그인</button>`;
}
...
function renderShell(route) {
  ...
  const renderNav = (className = "nav-list") => navItems.map(([href, icon, label, name]) => `<li><a class="nav-link" href="#${href}" data-route="${href}" data-nav-name="${name}" aria-current="${navIsActive(route, name) ? "page" : "false"}"><span class="nav-icon" aria-hidden="true">${icon}</span><span>${label}</span></a></li>`).join("");
  document.querySelector("#app").innerHTML = `<div class="research-shell">
    <aside class="sidebar" aria-label="주요 메뉴">
      <a class="sidebar-brand" href="#/market" data-route="/market" aria-label="시그널랩 홈">
        <span class="sidebar-brand-mark" aria-hidden="true">SL</span><span><strong>시그널랩</strong><small>리서치 데스크</small></span>
      </a>
      <div class="sidebar-scroll">
        <div class="sidebar-section"><p class="nav-group-label">탐색</p><nav aria-label="데스크톱 주요 메뉴"><ul class="nav-list">${renderNav()}</ul></nav></div>
      </div>
      <div class="sidebar-footer"><div class="sidebar-note"><span class="status-dot" aria-hidden="true"></span><div><strong>데이터 상태를 확인하세요</strong><span>기준 시각과 출처는 각 화면에 표시됩니다.</span></div></div></div>
    </aside>
    <div class="app-frame">
      <header class="topbar">
        <div class="topbar-context"><span class="context-dot" aria-hidden="true"></span><span>국내 주식 리서치</span></div>
        <form class="global-search" data-search-form role="search">
          <label class="sr-only" for="global-search-input">종목명 또는 코드 검색</label>
          <input id="global-search-input" name="q" value="${escapeHtml(state.searchQuery)}" placeholder="종목명 또는 코드 검색" autocomplete="off" ${searchInputAttributes(route)} />
          <button type="submit" aria-label="검색">검색</button>
        </form>
        <div class="topbar-actions" data-topbar-actions>${accountActionsMarkup()}<a class="btn btn-primary" href="#/subscription" data-route="/subscription">구독 안내</a></div>
      </header>
      <div class="market-rail market-tape" aria-label="시장 테이프" role="region">
        <div class="market-tape-inner">
          <span class="tape-item tape-lead"><span class="tape-kicker">시장 상태</span><strong>국내 개별주식</strong></span>
          <span class="tape-item"><span>장 상태</span><strong data-tape="session">확인 중</strong></span>
          <span class="tape-item"><span>KOSPI · KOSDAQ</span><strong data-tape="index">기준 시각 확인 중</strong></span>
          <span class="tape-item"><span>데이터 출처</span><strong data-tape="source">확인 중</strong></span>
          <span class="tape-item"><span>기준 시각</span><strong data-tape="asOf">본문 확인</strong></span>
        </div>
      </div>
      <div id="session-notice-root"></div>
      <nav class="bottom-nav" aria-label="모바일 주요 메뉴"><ul>${renderNav("bottom-nav-list")}</ul></nav>
      <main id="main-content" class="main" tabindex="-1"><div class="main-inner"><div class="skeleton" aria-label="페이지를 불러오는 중"></div></div></main>
    </div>
  </div>`;
  currentRouteKey = routeKey(route);
  renderSessionNotice();
}

function syncShell(route) {
  document.querySelectorAll("[data-nav-name]").forEach((link) => {
    link.setAttribute("aria-current", navIsActive(route, link.dataset.navName) ? "page" : "false");
  });
  const searchInput = document.querySelector("[data-search-input]");
  if (searchInput) {
    searchInput.value = state.searchQuery;
    searchInput.setAttribute("aria-expanded", String(route.name === "market" && state.searchResults.length > 0));
  }
  const actions = document.querySelector("[data-topbar-actions]");
  if (actions) actions.innerHTML = `${accountActionsMarkup()}<a class="btn btn-primary" href="#/subscription" data-route="/subscription">구독 안내</a>`;
  currentRouteKey = routeKey(route);
  renderSessionNotice();
}

function isCurrentRender(renderId, route) {
  if (renderId !== renderSequence) return;
  return routeKey(route) === routeKey(routeFromHash());
}
...
async function renderMarket(renderId) {
  ...
  try {
    ...
    main.innerHTML = `<section class="market-hero" aria-labelledby="market-title"><div><p class="eyebrow">국내 주식시장</p><h1 id="market-title">오늘 시장은 어디로 기울었나</h1><p class="lede">장중 숫자를 먼저 읽고, 움직임이 생긴 종목의 근거로 내려갑니다.</p></div><div class="market-hero-meta"><span class="hero-session">${escapeHtml(overview.session)}</span><span>기준 ${escapeHtml(formatDateTime(overview.asOf))}</span><span>${escapeHtml(sourceLabel(overview.source))}</span></div></section>${freshnessStrip(overview)}
      <section class="market-pulse" aria-labelledby="pulse-title"><div class="pulse-lead"><div class="section-kicker"><span>시장 흐름</span><span>시장 판독</span></div><h2 id="pulse-title">국내 시장의 무게중심은 <strong class="pulse-bias ${changeClass(leadIndex?.change)}">${leadBias}</strong>입니다.</h2><p>지수 방향과 시장 폭을 함께 보면 지금 숫자가 몇 종목에 의한 움직임인지 빠르게 확인할 수 있습니다.</p><div class="pulse-index-list" aria-label="주요 시장 지수">${indexRows}</div></div><div class="pulse-readout"><p class="readout-label">시장 폭</p><div class="breadth-summary"><strong>${escapeHtml(metric("상승 종목"))}</strong><span>상승</span><strong>${escapeHtml(metric("하락 종목"))}</strong><span>하락</span></div><div class="breadth-meter" role="img" aria-label="상승 ${escapeHtml(metric("상승 종목"))}, 하락 ${escapeHtml(metric("하락 종목"))}"><span class="breadth-up" style="width:${advanceWidth}%"></span><span class="breadth-down" style="width:${declineWidth}%"></span></div><div class="breadth-legend"><span><i class="legend-dot up" aria-hidden="true"></i>상승 ${escapeHtml(metric("상승 종목"))}</span><span><i class="legend-dot down" aria-hidden="true"></i>하락 ${escapeHtml(metric("하락 종목"))}</span></div><dl class="pulse-facts"><div><dt>거래대금</dt><dd>${escapeHtml(metric("거래대금"))}</dd></div><div><dt>외국인 수급</dt><dd class="${metricNumber("외국인 수급") >= 0 ? "up" : "down"}">${escapeHtml(metric("외국인 수급"))}</dd></div></dl></div></section>
      <section class="section market-workbench"><div class="movers-board" aria-labelledby="movers-title"><div class="board-heading"><div><p class="section-kicker">움직이는 종목</p><h2 id="movers-title">지금 움직이는 종목</h2><p>가격보다 먼저 변화를 발견하고, 종목 상세에서 근거를 확인하세요.</p></div><span class="board-heading-actions"><a class="text-link" href="#/screener" data-route="/screener">조건으로 더 찾기</a><span class="board-asof">${escapeHtml(formatDateTime(overview.asOf))}</span></span></div><div class="table-wrap"><table><caption class="sr-only">주요 종목 시세와 신선도</caption><thead><tr><th scope="col">순위</th><th scope="col">종목</th><th scope="col" class="numeric">가격</th><th scope="col" class="numeric">등락률</th><th scope="col">데이터 상태</th></tr></thead><tbody>${moverRows}</tbody></table></div></div><aside class="evidence-column"><section class="evidence-panel search-panel-wrap"><div class="panel-label">시장 검색</div><h2>읽고 싶은 종목을 찾으세요.</h2><p>종목명이나 6자리 코드로 국내 개별주식을 검색합니다.</p><form class="search-panel" data-search-form><label class="sr-only" for="market-search">종목명 또는 코드</label><input id="market-search" name="q" value="${escapeHtml(state.searchQuery)}" placeholder="예: 삼성전자 또는 005930" ${searchInputAttributes(route)} /><button class="btn btn-primary" type="submit">검색</button></form><p class="search-hint">화살표로 결과를 이동하고 Enter로 엽니다.</p>${searchBlock}</section><section class="evidence-panel"><div class="panel-label">근거 읽기</div><h2>근거는 숫자 옆에 있습니다.</h2><ul class="evidence-principles"><li><strong>기준 시각</strong><span>값이 언제의 것인지 먼저 확인</span></li><li><strong>출처</strong><span>예시 데이터 · 공급자 상태 확인 필요</span></li><li><strong>다음 읽기</strong><span>종목 → 가격 → 수급·뉴스·재무</span></li></ul><a class="text-link" href="#/signals" data-route="/signals">시그널 제공 범위 확인</a></section></aside></section>
      <section class="section scope-strip"><div><p class="section-kicker">리서치 범위</p><h2>국내 개별주식만 다룹니다.</h2></div><p>주문·계좌·자산관리·ETF/ETN/펀드는 제공하지 않습니다. 데이터가 지연되거나 오래된 경우 최신 상태로 해석하지 마세요.</p></section><p class="footer-note">출처: ${escapeHtml(sourceLabel(overview.source))} · 시장 기준 시각 ${escapeHtml(formatDateTime(overview.asOf))} · 플랫폼 수신 ${escapeHtml(formatDateTime(overview.receivedAt || overview.asOf))}</p>`;
  } catch (errorValue) {
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("MARKET OVERVIEW", "오늘의 시장")}${apiErrorMarkup(errorValue, "/market")}`;
  }
}
...
function lockedSignal(ticker, name, access = "subscription") {
  const loginRequired = access === "login";
  const heading = loginRequired ? `${escapeHtml(name)}은 로그인이 필요합니다` : `${escapeHtml(name)} 신호는 구독 권한이 필요합니다`;
  const description = loginRequired
    ? "로그인하면 저장·재방문·조건 실행·구독 상태 확인을 이어갈 수 있습니다."
    : "실제 방향·강도·정확한 발생 시각·근거·알고리즘 버전은 활성 <strong>REALTIME_SIGNAL</strong> 권한이 있을 때만 반환됩니다.";
  const details = loginRequired
    ? "<li>관심종목 저장과 재방문</li><li>승인된 조건 스크리너 실행</li><li>본인 구독·권한 상태 확인</li>"
    : "<li>데이터 신선도와 기준 시각</li><li>신호 상태와 근거 snapshot</li><li>신호 이력과 실시간 스트림</li>";
  const action = loginRequired
    ? `<button class="btn btn-primary" type="button" data-action="sandbox-login">로그인하고 계속</button>`
    : `<a class="btn btn-primary" href="#/subscription" data-route="/subscription">구독 상태 확인</a>${state.role === "guest" ? `<button class="btn btn-secondary" type="button" data-action="sandbox-login">로그인</button>` : ""}`;
  return `<div class="signal-lock"><span class="signal-lock-icon" aria-hidden="true">▣</span><div><h3>${heading}</h3><p>${description}</p></div><ul>${details}</ul><div class="page-actions">${action}</div></div>`;
}
...
function evidenceStockMarkup({ stock, quote, chart, flows, news, financials, signal, ticker, watchlistAction, quoteView }) {
  ...
  const flowSection = flows
    ...
    : card("수급", `<div class="signal-lock"><h3>로그인 후 수급을 확인할 수 있습니다</h3><p>회원 권한이 필요한 리서치 데이터입니다.</p><button class="btn btn-secondary" type="button" data-action="sandbox-login">로그인</button></div>`, "evidence-card", "flows");
  const newsSection = news
    ? card("뉴스", `<ul class="news-list">${news.items.map((item) => `<li><a href="#news" aria-label="${escapeHtml(item.title)}">${escapeHtml(item.title)}</a><span class="news-meta">${escapeHtml(sourceLabel(item.source))} · ${escapeHtml(formatDateTime(item.publishedAt))}</span></li>`).join("")}</ul><p class="footer-note">${freshness(news.dataStatus, news.asOf, news.source)}</p>`, "evidence-card", "news")
    : "";
...
async function renderStock(renderId, ticker) {
  ...
  try {
    ...
    if (!financials && state.role === "guest") {
      main.querySelector(".content-grid > .stack:last-child")?.insertAdjacentHTML("afterbegin", card("재무 요약", `<div class="signal-lock"><h3>로그인 후 재무를 확인할 수 있습니다</h3><p>회원 권한이 필요한 리서치 데이터입니다.</p><button class="btn btn-secondary" type="button" data-action="sandbox-login">로그인</button></div>`));
    }
    ...
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("STOCK RESEARCH", "종목 상세")}${apiErrorMarkup(errorValue, `#/stocks/${ticker}`)}`;
  }
}
...
async function renderSignals(renderId) {
  const main = document.querySelector("#main-content .main-inner");
  const route = routeFromHash();
  try {
    ...
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("REALTIME SIGNALS", "실시간 시그널")}${apiErrorMarkup(errorValue, "/signals")}`;
  }
}

async function renderScreener(renderId) {
  const main = document.querySelector("#main-content .main-inner");
  const route = routeFromHash();
  if (state.role === "guest") {
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("STRUCTURED SCREENER", "조건 스크리너", "승인된 구조화 조건으로 종목을 찾습니다.")}${lockedSignal("", "조건 스크리너", "login")} `;
    return;
  }
  try {
...
async function renderWatchlist(renderId) {
  const main = document.querySelector("#main-content .main-inner");
  const route = routeFromHash();
  if (state.role === "guest") {
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("WATCHLIST", "관심종목", "저장·재방문·삭제는 회원 기능입니다.")}${lockedSignal("", "관심종목", "login")} `;
    return;
  }
  try {
...
function subscriptionActionMarkup(subscription, entitlementActive, pending) {
  ...
  return `<button class="btn btn-primary" type="button" data-action="checkout">구독 시작</button>`;
}

async function renderSubscription(renderId) {
  const main = document.querySelector("#main-content .main-inner");
  const route = routeFromHash();
  if (state.role === "guest") {
    if (!isCurrentRender(renderId, route)) return;
    main.innerHTML = `${pageHeading("SUBSCRIPTION & ACCOUNT", "구독·계정", "로그인과 구독 권한 상태를 확인하는 화면입니다.")}${lockedSignal("", "구독·계정", "login")} `;
    return;
  }
  try {
...
function updateLiveSignal(signal) {
  const route = routeFromHash();
  if (route.name !== "signals" && route.name !== "stock") return;
  if (state.stream.ticker && signal.ticker && state.stream.ticker !== signal.ticker) return;
...
async function render() {
  ...
  const routeChanged = currentRouteKey !== nextRouteKey;
  const renderId = ++renderSequence;
  closeStream();
  if (!document.querySelector(".research-shell")) renderShell(route);
  else syncShell(route);
  if (routeChanged) {
    const main = document.querySelector("#main-content");
    main.querySelector(".main-inner").innerHTML = '<div class="skeleton" aria-label="페이지를 불러오는 중"></div>';
    main.focus({ preventScroll: true });
  }
  currentRouteKey = nextRouteKey;
  if (renderId !== renderSequence) return;
  if (route.name === "market") return renderMarket(renderId);
  if (route.name === "signals") return renderSignals(renderId);
  if (route.name === "screener") return renderScreener(renderId).finally(() => { if (renderId === renderSequence && state.screenerError) document.querySelector("[data-screener-form]")?.setAttribute("aria-describedby", "screener-error"); });
  if (route.name === "watchlist") return renderWatchlist(renderId);
  if (route.name === "subscription") return renderSubscription(renderId);
  if (route.name === "stock") return renderStock(renderId, route.ticker);
}
